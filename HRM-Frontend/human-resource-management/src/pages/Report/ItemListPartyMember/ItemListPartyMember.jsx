...
function ItemListPartyMember() {
  ...
  const handleClick = async () => {
    await ProductApi.PostLS({
      ...
      thaoTac: `Tải về file báo cáo danh sách đảng viên`,
  ...
  const handleClickPdf = async () => {
    await ProductApi.PostLS({
      tenTaiKhoan: decoded.userName,
      thaoTac: `Tạo file báo cáo danh sách đảng viên`,
      maNhanVien: decoded.id,
      tenNhanVien: decoded.givenName,
    });
  };
  ...
  const handelReport = async () => {
    if (check === "Tất cả") {
      try {
        const resp = await ProductApi.getRpAllDv();
        setDataRp(resp);
      } catch (e) {
      ...
      if (department === "Tất cả") {
        ...
        try {
          const resp = await ProductApi.getRpAllDvPb(department);
          setDataRp(resp);
        } catch (e) {
          error("Thực hiện không thành công");
        }
...
export default ItemListPartyMember;
